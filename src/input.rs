...
#[allow(dead_code)]
pub fn take_int<T>() -> T where T: FromStr, <T as FromStr>::Err: std::fmt::Debug {
    _take_int_from_reader(std::io::stdin())
}

#[allow(dead_code)]
pub fn take_2int<T1, T2>() -> (T1, T2) 
where 
...
fn _take_int_from_reader<T, R>(reader: R) -> T where T: FromStr, <T as FromStr>::Err: std::fmt::Debug, R: Read {
...
fn _take_2int_from_reader<T1, T2, R>(reader: R) -> (T1, T2)  
where 
    T1: FromStr, <T1 as FromStr>::Err: std::fmt::Debug, 
    T2: FromStr, <T2 as FromStr>::Err: std::fmt::Debug, 
    R: Read
